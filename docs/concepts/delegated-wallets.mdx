...
2. DeCash API sends the Algorand transaction to the owner to sign.
3. The owner signs the transaction thus allowing Delegated Wallet's Smart Signature to verify it, and sends the
   transaction back to DeCash API.
...
+ The provider's account used to [opt in](/docs/concepts/tokens#token-opt-in) and opt out this Delegated Wallet,
	which will manage Algo currency on this wallet required by Algorand to have it operational.
