...
It is very important to check if the transaction has been finalized by checking the (TransactionStatus)[/docs/api/algorand/transaction-status] endpoint after submission to avoid double spending. (Transactions are usually finalized in less than 10 seconds.)
