...
| `sender`    | `string`                                  | The address of the account that is sending the funds. Note that this account must have enough funds to cover both all the `amount`s, and the [fees](/docs/api/fees). **This array can be 4 addresses at max**. |
